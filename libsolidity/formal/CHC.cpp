...
void CHC::endVisit(ContractDefinition const& _contract)
{
	...
	addRule(smtutil::Expression::implies(initialConstraints(_contract), predicate(entry)), entry.functor().name);
	...
	for (auto base: _contract.annotation().linearizedBaseContracts | boost::adaptors::reversed)
	{
		...
		auto const& afterBase = *createConstructorBlock(_contract, "implicit_constructor_after_base_" + base->name() + "_" + to_string(base->id()));
...
smtutil::Expression CHC::initialConstraints(ContractDefinition const& _contract, FunctionDefinition const* _function)
{
	...
	FunctionDefinition const* function = _function ?
		_function :
