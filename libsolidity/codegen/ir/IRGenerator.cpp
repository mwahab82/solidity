...
string IRGenerator::generateFunction(FunctionDefinition const& _function, bool _includeModifiers)
{
	string functionName = _includeModifiers ? IRNames::function(_function) : IRNames::functionWithModifierInner(_function);
	return m_context.functionCollector().createFunction(functionName, [&]() {
		...
		for (auto const& varDecl: _function.returnParameters())
		{
			...
			if (!_includeModifiers || _function.modifiers().empty())
		...
		else
		{
			for (size_t i = 0; i < _function.modifiers().size(); ++i)
			{
				...
				string next =
					...
					IRNames::modifierInvocation(*_function.modifiers().at(i)) :
			...
			generateFunction(_function, false);
		}
		return t.render();
	});
}
...
)
{
	...
	return m_context.functionCollector().createFunction(functionName, [&]() {
		...
		vector<string> retParams;
		for (auto const& varDecl: _function.returnParameters())
			retParams += IRVariable(*varDecl).stackSlots();
		t("retParams", joinHumanReadable(retParams));
